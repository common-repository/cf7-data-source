	...
	public function wpcf7_tag_generator_datatable( $contact_form, $args = '' ) {
		...
		jQuery(function(){
			...
			function populate_datalists() {
				...
				{
					let form_structure = form_editor.val(),
			...
			populate_datalists();
			$(document).on('mousedown', '[href*="datatable"]', function(){ populate_datalists(); });
		});
		</script>
		...
		<div class="insert-box" style="display:flex;overflow:hidden;">
			<input type="text" name="datatable" class="tag code" readonly="readonly" onfocus="this.select();" style="margin-right:5px;" />
