...
if ( ! class_exists( 'CF7_DATASOURCE_BASE' ) ) {
	class CF7_DATASOURCE_BASE {
		...
		private $constants;
		private $manager;
		protected $timeout = 3600; // Used with wp_remote_get, wp_remote_post, and wp_remote_request;
		...
		public function __construct( $manager ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				foreach ( $columns as $column ) {
					$this->constants[ 'post.' . $column ] = null;
				}
		...
		protected function default_attributes() {
			return array(
				...
				'debugging_email' => '', // Email address where receive the error logs.
