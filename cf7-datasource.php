...
if ( ! class_exists( 'CF7_DATASOURCE' ) ) {
	class CF7_DATASOURCE {
		...
		public static function init() {
			// This patch allows loading dynamic data in dropdown and checkbox fields.
			remove_action(	'wpcf7_swv_create_schema',	'wpcf7_swv_add_select_enum_rules', 20 );
			remove_action(	'wpcf7_swv_create_schema',	'wpcf7_swv_add_checkbox_enum_rules', 20 );
		...
		public function get_data( $data ) {
			...
			if ( false != $attrs ) {
				...
				$this->debugging_email = ! empty( $attrs['debugging_email'] ) && '' != ( $email_address = sanitize_email( $attrs['debugging_email'] ) ) ? $email_address : '';
			...
			if ( ! empty( $mssg ) ) {
				...
				if ( ! empty( $this->debugging_email ) ) {
					try {
						...
						$subject = __( 'Data Source for Contact Form 7 - Debugging Email', 'cf7-datasource' );
						$blog_name = get_option( 'blogname', '' );
						if ( ! empty( $blog_name ) ) {
							$subject .= ' (' . $blog_name . ')';
						}
						wp_mail(
							$this->debugging_email,
							$subject,
							$mssg,
							$headers
						);
		...
		public function validL() {
			...
			if ( $l ) {
				...
				try {
					...
					if ( is_array( $contents ) && ! empty( $contents['body'] ) && false !== stripos( $contents['body'], 'not valid' ) ) {
